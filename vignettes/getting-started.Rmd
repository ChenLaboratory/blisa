...
plotHotspots(BLISAres, index = 1)

# Or by gene names:
# plotHotspots(BLISAres, ligand = "CXCL12", receptor = "CXCR4")
```
...
plotCCI(BLISAres, top_lr = 20, top_pairs = 30)
```
...
```{r fig.width=6, fig.height=5}
plotCCILR(BLISAres, ligand = "CXCL12", receptor = "CXCR4")
```
...
```{r fig.width=6, fig.height=5}
plotCCIsummary(BLISAres)

# With a different aggregation:
# plotCCIsummary(BLISAres, agg_fun = mean)
```
